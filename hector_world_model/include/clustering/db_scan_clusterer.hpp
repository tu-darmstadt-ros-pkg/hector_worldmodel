...
{
public:
  DBScanClusterer( std::atomic<int> &latest_marker_id, std::shared_ptr<WorldModel> node );

  ~DBScanClusterer() noexcept { };
...
private:
  ...
  std::vector<int> calculateCluster( const Point &source_point );
  inline bool isWithinEpsilon( const Point &p1, const Point &p2 );
  inline double d( const Point &p1, const Point &p2 );
